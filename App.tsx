...
import { LottoResult, PredictionResult, Pattern, HybridPatternInfo, RepeatAnalysis, BacktestResult } from './types';
...
const App: React.FC = () => {
  ...
  const autoCalculate = () => {
    ...
    allPredictions.forEach(pred => {
      ...
      const currentAcc = hybridInfo?.currentStats.directAccuracy || 0;
      const historicalAcc = hybridInfo?.historicalStats.directAccuracy || 0;
      const stability = hybridInfo?.stabilityScore || 0;
      const learningBonus = ruleBiases[pred.name] || 1.0;
      
      // ✅ NEW: 70/30 Dynamic Weighting (Prioritize Recent Performance)
      let weight = (currentAcc * 0.7) + (historicalAcc * 0.3);
      
      // ✅ NEW: Dead Formula Penalty (If 10-round accuracy is 0%, cut weight by 90%)
      if (currentAcc === 0) {
        weight *= 0.1;
      }
      ...
      // Apply learning bonus (from ruleBiases)
      ...
      if (recent5Results.includes(pred.value)) {
        ...
        // Very cold numbers get a slight boost but not too much to avoid "gambler's fallacy"
        weight *= 1.2;
      }

      // ✅ NEW: Digit Recency Check
      // If a predicted digit hasn't appeared at all in the last 10 draws, penalize it
      const tens = pred.value[0];
      const units = pred.value[1];
      const recent10Results = allData.slice(0, 10).map(r => r.r2.padStart(2, '0'));
      const tensAppeared = recent10Results.some(r => r[0] === tens);
      const unitsAppeared = recent10Results.some(r => r[1] === units);
      
      if (!tensAppeared || !unitsAppeared) {
        weight *= 0.5; // Reduce weight if one of the digits is "Dead"
      }
